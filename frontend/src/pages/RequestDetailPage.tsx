  ...
  if (!request) {
    return (
      <div className="text-center py-20">
        ...
        <button onClick={() => navigate(-1)} className="mt-4 text-violet-600 hover:underline">← Retour</button>
      </div>
    );
  }
          ...
          <div className="space-y-2">
            {request.participants.map((p) => (
              <div key={p.id} className="flex justify-between items-center p-2 bg-gray-50 rounded">
                ...
                <span className="text-xs bg-violet-100 text-violet-700 px-2 py-0.5 rounded">{p.participantStatus}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border p-6">
          ...
          <div className="space-y-4 mb-4">
            {comments.map((c) => (
              <div key={c.id} className={`flex items-start gap-3 ${c.authorId === user?.id ? 'flex-row-reverse' : ''}`}>
                ...
                <div className={`max-w-[75%] ${c.authorId === user?.id ? 'bg-violet-50 border-violet-100' : 'bg-gray-50 border-gray-100'} border rounded-lg p-3`}>
        ...
        <div className="flex gap-2">
          <input
            ...
            className="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500 focus:border-transparent"
          />
          <button
